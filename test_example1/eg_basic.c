...
ECS_COMPONENT_DECLARE(EgState);
ECS_COMPONENT_DECLARE(EgAction);
...
void FlecsComponentsBasicImport(ecs_world_t *world)
{
	...
	ECS_COMPONENT_DEFINE(world, EgState);
	ECS_COMPONENT_DEFINE(world, EgAction);
	...
	ecs_struct_init(world, &(ecs_struct_desc_t) {
	.entity.entity = ecs_id(EgState),
	.members = {
	{ .name = "status", .type = ecs_id(ecs_u32_t) },
	{ .name = "text", .type = ecs_id(ecs_string_t) },
	}
	});

	ecs_struct_init(world, &(ecs_struct_desc_t) {
	.entity.entity = ecs_id(EgAction),
	.members = {
	{ .name = "action", .type = ecs_id(ecs_u32_t) },
	{ .name = "text", .type = ecs_id(ecs_string_t) },
	}
	});
